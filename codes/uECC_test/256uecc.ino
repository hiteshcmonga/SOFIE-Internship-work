...
Serial.println("---");
        a = millis();  
        int x=uECC_verify(public1, encoded, hash_size,sign1, curve);
        Serial.printf("Check against public1 = %d\n", x);
        b = millis();
         Serial.print("check against public one done in "); 
         Serial.println(b-a);
          if (!x) {
          Serial.print("check1() failed (2)\n");
          return;
           }  
        a = millis();          
        int y=uECC_verify(public2, encoded, hash_size,sign1, curve);
